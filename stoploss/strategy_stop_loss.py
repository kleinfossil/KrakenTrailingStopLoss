...
def update_stop_loss_trigger2(position, std_interval="d", std_history=10, minmax_interval="h", minmax_history=24):
    ...
    try:
        if position.trigger > 0:
            ...
            raise RuntimeError(f"Position Trigger is not larger 0 (value is {position.trigger}. You tried to execute the update of a stop_loss trigger with an active position. Try initiate instead.")
        return stop_loss_position
    except RuntimeError as e:
        logger.error(f"{traceback.print_stack()} {e}")
